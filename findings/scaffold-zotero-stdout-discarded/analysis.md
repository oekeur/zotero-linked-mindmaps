...
**Status 2026-09-11:** fixed for `serve` in `zotero-plugin-scaffold` 0.9.1 (2026-08-31). `server.debugOutputFile` (default `true`) appends `-ZoteroDebugText` and writes stdout and stderr to `.scaffold/logs/zotero-<time>.log` and `zotero-<time>-stderr.log`; `server.debugOutputWindow` opens the Debug Output window. The test runner (`src/core/tester/index.ts`) does not pass the option to its `ZoteroRunner`, so a test run's output is still drained into an empty handler. The remainder of this file describes 0.8.8, which this project still installs.
