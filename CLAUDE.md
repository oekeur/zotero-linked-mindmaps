...
- **Manual verification protocol** (for any change touching XUL, Cytoscape, or live Zotero API — nothing here is unit-testable, and most failure modes in this codebase so far have been *silent*, not thrown): before declaring such a change done —
  ...
  3. After `npm start`, don't trust "Installed as temporary add-on" alone: the initial temp-install can swallow a `ReferenceError` or other startup exception while still reporting success. Trigger one hot-reload (save any watched file) to force `bootstrap.js`'s `startup()` to actually re-run, then check Debug Output (Help → Debug Output) for "Error running bootstrap method" before trusting the plugin initialized.
