...
<!--- Delete the block below if this PR touches no XUL, Cytoscape, or live Zotero API code.
      Nothing in that surface is unit-testable and most failures here are silent. -->

Touches XUL, Cytoscape, or live Zotero API code, so also:

- [ ] Driven manually via `npm start`, not just built
- [ ] "Zotero Linked Mindmaps" still appears under Tools -> Plugins
