 ...
 * -> Debug Output Logging), so anything logged through it alone is gone by
 * the time a bug is reported. Zotero.logError additionally reaches the
 * Mozilla error console (Zotero.getErrors()), which Zotero records
 * unconditionally - that's the channel failures need to survive on.
 */

/**
 * Logs a genuine failure through Zotero.logError. `message` should already
 * carry the `[zoteroLinkedMindmaps]` prefix and any relevant detail, matching
 * the convention at existing call sites. Zotero.logError only forwards
