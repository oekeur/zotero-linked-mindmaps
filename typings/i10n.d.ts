// Generated by zotero-plugin-scaffold
/* prettier-ignore */
/* eslint-disable */
// @ts-nocheck
export type FluentMessageId =
  | 'add-link-button'
  | 'add-link-choose-external-button'
  | 'add-link-choose-target-button'
  | 'add-link-dialog-context'
  | 'add-link-direction-backward'
  | 'add-link-direction-forward'
  | 'add-link-direction-label'
  | 'add-link-external-none'
  | 'add-link-name-label'
  | 'add-link-save-button'
  | 'add-link-save-button-disabled'
  | 'add-link-self-link-error'
  | 'add-link-target-empty'
  | 'add-link-target-label'
  | 'add-link-target-not-linkable'
  | 'add-link-type-label'
  | 'add-link-window'
  | 'add-to-mindmap-progress'
  | 'container-trashed-now'
  | 'container-trashed-startup'
  | 'cross-library-refused'
  | 'group-on-mindmap-dialog-message'
  | 'group-on-mindmap-dialog-title'
  | 'group-on-mindmap-progress'
  | 'group-on-mindmap-skipped'
  | 'item-mindmaps-add-link-header-button'
  | 'item-mindmaps-choose-mindmap-continue'
  | 'item-mindmaps-choose-mindmap-label'
  | 'item-mindmaps-current-label'
  | 'item-mindmaps-edit-link-button'
  | 'item-mindmaps-empty-state'
  | 'item-mindmaps-error-state'
  | 'item-mindmaps-no-links-state'
  | 'item-mindmaps-remove-from-group-button'
  | 'item-mindmaps-remove-from-named-group-button'
  | 'item-mindmaps-remove-link-button'
  | 'item-mindmaps-remove-node-button'
  | 'item-mindmaps-section-head-text'
  | 'item-mindmaps-section-sidenav-tooltip'
  | 'itemmenu-add-link'
  | 'itemmenu-add-link-submenu'
  | 'itemmenu-add-to-mindmap'
  | 'itemmenu-group-on-mindmap'
  | 'itemmenu-group-on-mindmap-submenu'
  | 'link-type-default-cites'
  | 'link-type-default-contradicts'
  | 'link-type-default-primary-source-for'
  | 'link-type-default-related-to'
  | 'link-type-default-supports'
  | 'menuitem-mindmap-open'
  | 'mindmap-data-trashed-open'
  | 'mindmap-delete-button'
  | 'mindmap-delete-confirm-message'
  | 'mindmap-delete-confirm-title'
  | 'mindmap-dock-close'
  | 'mindmap-edit-button'
  | 'mindmap-empty-state'
  | 'mindmap-fit-button'
  | 'mindmap-form-cancel-button'
  | 'mindmap-form-description-label'
  | 'mindmap-form-save-button'
  | 'mindmap-form-title-label'
  | 'mindmap-group-create'
  | 'mindmap-group-delete'
  | 'mindmap-group-rename'
  | 'mindmap-legend-directional'
  | 'mindmap-legend-external-node'
  | 'mindmap-legend-group-membership'
  | 'mindmap-legend-group-region'
  | 'mindmap-legend-heading'
  | 'mindmap-legend-parent-child-tie'
  | 'mindmap-legend-toggle-button'
  | 'mindmap-legend-undirectional'
  | 'mindmap-legend-unknown-type'
  | 'mindmap-new-button'
  | 'mindmap-relayout-button'
  | 'mindmap-relayout-confirm-all'
  | 'mindmap-relayout-confirm-selection'
  | 'mindmap-relayout-confirm-title'
  | 'mindmap-show-in-library'
  | 'mindmap-sidebar-collapse'
  | 'mindmap-sidebar-expand'
  | 'mindmap-sidebar-heading'
  | 'mindmap-sidebar-new-button'
  | 'mindmap-tab-title'
  | 'mindmap-zoom-in-button'
  | 'mindmap-zoom-out-button'
  | 'preferences-add-button'
  | 'preferences-cancel-button'
  | 'preferences-column-directional'
  | 'preferences-column-label'
  | 'preferences-delete-button'
  | 'preferences-delete-confirm-title'
  | 'preferences-delete-confirm-unknown'
  | 'preferences-delete-confirm-used'
  | 'preferences-edit-button'
  | 'preferences-feedback-bug-button'
  | 'preferences-feedback-feature-button'
  | 'preferences-feedback-heading'
  | 'preferences-feedback-help'
  | 'preferences-field-directional'
  | 'preferences-field-label'
  | 'preferences-heading'
  | 'preferences-hide-mindmap-notes'
  | 'preferences-hide-mindmap-notes-help'
  | 'preferences-library-heading'
  | 'preferences-pane-label'
  | 'preferences-save-button'
  | 'preferences-type-directional'
  | 'preferences-type-undirected'
  | 'startup-begin'
  | 'startup-finish'
  | 'storage-note-trashed-now';
