# Mindmaps section reference

An item-pane section headed "Mindmaps" that shows which mindmap the selected item belongs to, lists the links touching it, and offers controls to add or remove them. The sidenav icon's tooltip reads "Mindmaps".
...
Links in the mindmaps it did not pick are not shown. Where the item is a node in more than one, the mindmap name at the top is a dropdown listing every mindmap holding it, and choosing another redraws the panel against that one. An item in a single mindmap gets its name as plain text instead, since there would be nothing to pick.
...
Either control writes to the mindmap the panel is showing, the one named at the top. The panel records it on the form when it draws, so the header "+" reaches it too even though the item pane hands that button nothing but the panel body.
...
- [mindmaps-panel-howto.md](mindmaps-panel-howto.md)
