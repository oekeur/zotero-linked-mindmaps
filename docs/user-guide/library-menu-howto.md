...
Attachments in the selection get skipped, as do the plugin's own "Zotero Linked Mindmaps (plugin data)" item and the mindmap notes under it. Select nothing but those and the entry still appears; clicking it reports "Added 0 item(s) to mindmap".
...
The Connections panel is the other route onto a specific mindmap, and it's the one to use when you want to author a link at the same time ([connections-panel-howto.md](connections-panel-howto.md)).
