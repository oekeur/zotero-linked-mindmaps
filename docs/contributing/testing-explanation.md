...
Almost everything the plugin does is a call into Zotero. Storage is a note item carrying a JSON blob, found by tag, saved through `saveTx()` inside a transaction, and hung off a container item. Deletion cleanup listens to `Zotero.Notifier`. The graph tab is a `Zotero_Tabs` entry. The Connections panel is registered through `itemPaneManager`. A mock of that surface would amount to a second implementation of Zotero's data layer, and the failures worth catching are precisely the ones where Zotero's real behavior differs from what the code assumed.
