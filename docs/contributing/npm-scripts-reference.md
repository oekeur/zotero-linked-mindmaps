...
Two cleanups against this checkout's dev profile. The path comes from an exported `ZOTERO_PLUGIN_PROFILE_PATH` if there is one, otherwise from `.env` -- the same precedence the scaffold uses, so the script always aims at the instance `npm start` actually launched:
...
2. Takes the same resolved profile path, opens `<profile>/session.json`, and drops every tab whose `type` starts with `zoterolinkedmindmaps-` (the `config.addonRef` value plus a hyphen). If it removed tabs and none of the survivors is marked `selected`, it selects the last remaining tab, then rewrites `session.json`.
